...
public:
  ...
  friend quaternion operator*(T k, const quaternion& q) { return quaternion(q.w * k, q.x * k, q.y * k, q.z * k); }
  friend quaternion operator/(const quaternion& q, T k) { return quaternion(q.w / k, q.x / k, q.y / k, q.z / k); }

  quaternion operator-() const { return quaternion(w * -1, x * -1, y * -1, z * -1); }
  ...
  quaternion conjugate() const { return quaternion(w, x * -1, y * -1, z * -1); }

  // void check_divide_by_zero(double magn, char* x) {
  //   if (magn == 0)
  //     ASSERT("can't divide by 0");
  // }
...
void quaternion<T>::run_tests() {
  ...
  plane_rotation("straightDown(N)", quatD(0.5, -0.5, 0.5, -0.5),  {  0,  0,  1,  -1,  0,  0,   0, -1,  0 });
  plane_rotation("straightDown(N)", quatD(0, -rad2_2, rad2_2, 0), {  0, -1,  0,  -1,  0,  0,   0,  0, -1 } );
  plane_rotation("straightDown(S)", quatD(0.5, 0.5, -0.5, -0.5),  {  0,  0, -1,  -1,  0,  0,   0,  1,  0 });
